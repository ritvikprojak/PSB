...
public class DMSImpl2 {
	
	static Logger log = Logger.getLogger(DMSImpl2.class.getName());
	...
	public void syncWithDMS(ArrayList<BulkEmployeeDetails> bulkEmpDetails,String objectStore) {
		...
		for (int i = 0; i < bulkEmpDetails.size(); i++) {
			...
			if(status.equalsIgnoreCase("Active")){
				...
				addUserToRole(userId,objectStore,roleDesc);
		...
		if (ce.isConnected()) {
			...
			if (!roleName.isEmpty()) {
				...
				try {
					...
					if (e.getExceptionCode().equals(com.filenet.api.exception.ExceptionCode.E_OBJECT_NOT_FOUND)) {
	...
	public void addUserToRole(String userName, String objectStoreName, String roleName) {
